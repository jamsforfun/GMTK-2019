...
public static class ExtDrawGuizmos
{
    ...
    /// <param name="x"></param>
    /// <param name="y"></param>
    public static void DrawGUI(float x, float y, Color color)
    {
        int holdDepth = GUI.depth;

        GUI.depth = 500;
        GUIStyle backGround = new GUIStyle();
        backGround.normal.background = ExtTexture.MakeTex(600, 1, color);

        GUILayout.BeginArea(new Rect(x - 5, y - 5, 10, 10));
